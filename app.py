...
st.write("Rata rata lama sekolah di Indonesia dibandingkan dengan negara lain sangat jauh. German sebagai negara tertinggi dengan rata-rata lama sekolah pada tahun 2019 berada pada angka 14,13 tahun yang mengartikan rata-rata penduduk German telah menempuh pendidikan selama 14,13 tahun. Hasil ini sangat jauh dibandingkan Indonesia yang pada tahun 2019 mencapai 8,34 tahun selisih 5,79 tahun dengan rata-rata lama sekolah di German. Perkembangan rata-rata lama sekolah di Indonesia dari tahun ke tahun juga termasuk rendah dibandingkan dengan negara lain.")

st.subheader(
    'Rata-rata Lama Sekolah dan Persentase Penangguran di Indonesia')
df = pd.read_csv("Presentase pengangguran diindonesia.csv")
df['dates'] = pd.to_datetime(df['tahun'], format='%Y')
...
chart_rata_rata_lama_sekolah = alt.Chart(df).mark_line(point=True).encode(
    x=alt.X("dates", title="Tahun", axis=alt.Axis(format='%Y')),
    y="Rata rata lama sekolah (Tahun)"
).properties(
    title="Grafik Rata-rata Lama Sekolah (Tahun)"
)


chart_persentasi_pengangguran = alt.Chart(df).mark_line(point=True).encode(
    x=alt.X("dates", title="Tahun", axis=alt.Axis(
        format='%Y')),
    y=alt.Y("Presentase pengangguran (%)", scale=alt.Scale(domain=[0, 9]))
).properties(
    title="Grafik Persentase Pengangguran (%)"
)
col1, col2 = st.columns(2)

with col1:
    st.altair_chart(chart_rata_rata_lama_sekolah, use_container_width=True)

with col2:
    st.altair_chart(chart_persentasi_pengangguran, use_container_width=True)

# st.dataframe(new_df)

st.write("Grafik diatas menunjukkan rata rata lama sekolah di indonesia cenderung stabil selama 9 tahun terakhir. Hal tersebut tercermin meningkatnya rata-rata lama sekolah di Indonesia dari tahun ke tahun. Data-data lama sekolah di Indonesia dari tahun 2013 hingga 2021 hanya meningkat sebanyak 0,70% dari awalnya 7,61 tahun ke 8,54 tahun. Sedangkan persentase pengangguran mengalami fluktuasi selama sembilan tahun terakhir. Tahun 2015 menjadi awal dari menurunnya persentase pengangguran hingga pada tahun 2019. Tingkat perubahan signifikan persentase pengangguran terjadi pada tahun 2020 hingga 2021 yang telah diketahui pada periode tersebut merupakan awal dari munculnya COVID-19.")


st.subheader("Kesimpulan")
st.markdown(
    "1. Rata-rata lama sekolah di Indonesia memiliki korelasi rendah dengan persentase pengangguran di Indonesia.")
st.markdown(
